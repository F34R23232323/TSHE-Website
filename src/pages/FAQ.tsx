    ...
    <div style={{ position: 'relative', zIndex: 1, maxWidth: 820, margin: '0 auto', padding: '3rem 1.5rem 6rem' }}>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: '.5rem', marginBottom: '2rem', justifyContent: 'center' }}>
        {cats.map(c => (
          <button key={c} onClick={() => setCat(c)} style={{
            ...
            background: catFilter === c ? 'rgba(126,200,227,.12)' : 'var(--bg2)',
            border: `1px solid ${catFilter === c ? 'rgba(126,200,227,.3)' : 'var(--border)'}`,
            color: catFilter === c ? 'var(--ice2)' : 'var(--muted)',
            transition: 'all .15s',
          }}>{c}</button>
        ))}
      </div>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '.5rem' }}>
        {visible.map((item, i) => {
          ...
          return (
            <div key={key} style={{
              background: 'var(--bg1)', border: `1px solid ${open ? 'rgba(126,200,227,.2)' : 'var(--border)'}`,
              ...
              <button onClick={() => setActive(open ? null : key)} style={{
                ...
                <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'flex-end', gap: '.3rem', flexShrink: 0 }}>
                  <span style={{
                    fontSize: '.58rem', fontFamily: 'var(--display)', letterSpacing: '.06em',
                    padding: '2px 7px', borderRadius: 4,
                    background: 'rgba(126,200,227,.08)', border: '1px solid rgba(126,200,227,.15)',
                    color: 'var(--ice)', whiteSpace: 'nowrap',
                  }}>{item.category}</span>
                  ...
                  <p style={{ fontSize: '.8rem', color: 'var(--dim)', lineHeight: 1.75 }}>
                    {item.a.includes('http')
                      ? item.a.split(/(https?:\/\/\S+)/).map((part: string, j: number) =>
                          part.startsWith('http') ? <a key={j} href={part} target="_blank" rel="noreferrer" style={{ color: 'var(--ice)', textDecoration: 'underline' }}>{part}</a> : part
                        )
                      : item.a
                    }
                  </p>
