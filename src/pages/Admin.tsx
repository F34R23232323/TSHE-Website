...
import {
  ...
  getServices, saveServices, type ServiceItem,
...
export function MaintenanceScreen() {
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, zIndex: 9999, background: 'var(--bg)', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', padding: '2rem', textAlign: 'center' }}>
      <div style={{ width: 88, height: 88, borderRadius: '1.4rem', margin: '0 auto 2rem', background: 'linear-gradient(135deg,rgba(126,200,227,.15),rgba(201,168,76,.15))', border: '1px solid rgba(126,200,227,.25)', display: 'grid', placeItems: 'center', boxShadow: '0 0 60px rgba(126,200,227,.1)', animation: 'glowPulse 4s ease-in-out infinite' }}>
        <Wrench size={40} color="var(--ice2)" strokeWidth={1.5} />
      </div>
      ...
      <h1 style={{ fontFamily: '"Cinzel Decorative", serif', fontSize: 'clamp(1.8rem,4vw,2.8rem)', fontWeight: 700, marginBottom: '.75rem', background: 'linear-gradient(135deg,var(--ice3),var(--ice),var(--gold))', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>{cfg.serverName}</h1>
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', padding: '2rem', position: 'relative', zIndex: 1 }}>
      <div style={{ width: '100%', maxWidth: 380, background: 'var(--bg1)', border: '1px solid var(--border)', borderRadius: '1.25rem', padding: '2.5rem', textAlign: 'center' }}>
        <div style={{ width: 64, height: 64, borderRadius: '1rem', margin: '0 auto 1.25rem', background: 'linear-gradient(135deg,rgba(88,101,242,.2),rgba(126,200,227,.1))', border: '1px solid rgba(88,101,242,.3)', display: 'grid', placeItems: 'center' }}>
...
const css = {
  input: {
    ...
    background: '#0d1428', border: '1px solid rgba(126,200,227,.12)',
...
const onFocus = (e: React.FocusEvent<any>) => { e.target.style.borderColor = 'rgba(126,200,227,.5)'; e.target.style.boxShadow = '0 0 0 3px rgba(126,200,227,.06)' }
const onBlur  = (e: React.FocusEvent<any>) => { e.target.style.borderColor = 'rgba(126,200,227,.12)'; e.target.style.boxShadow = 'none' }

function F({ label, hint, children }: { label: string; hint?: string; children: React.ReactNode }) {
  return (
    <div style={{ marginBottom: '1rem' }}>
      <label style={{ display: 'block', fontFamily: 'Cinzel, serif', fontSize: '.62rem', letterSpacing: '.12em', color: 'rgba(126,200,227,.6)', marginBottom: '.4rem', textTransform: 'uppercase', fontWeight: 600 }}>{label}</label>
      {children}
...
function Toggle({ checked, onChange, label, desc, danger }: { checked: boolean; onChange: (v: boolean) => void; label: string; desc?: string; danger?: boolean }) {
  return (
    <div onClick={() => onChange(!checked)} style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', padding: '.85rem 1rem', background: checked && danger ? 'rgba(248,113,113,.05)' : 'rgba(255,255,255,.02)', border: `1px solid ${checked && danger ? 'rgba(248,113,113,.2)' : 'rgba(255,255,255,.05)'}`, borderRadius: '.65rem', marginBottom: '.65rem', cursor: 'pointer', transition: 'all .15s' }}>
      ...
      <div style={{ width: 44, height: 24, borderRadius: 99, background: checked ? (danger ? 'var(--red)' : '#7ec8e3') : 'rgba(255,255,255,.08)', transition: 'background .2s', flexShrink: 0, position: 'relative' }}>
...
function Btn({ onClick, variant = 'secondary', children, small }: { onClick: () => void; variant?: 'primary'|'secondary'|'danger'|'ghost'; children: React.ReactNode; small?: boolean }) {
  const v = {
    primary:   { background: 'rgba(126,200,227,.15)', border: '1px solid rgba(126,200,227,.3)',  color: 'var(--ice2)' },
...
function Section({ title, icon: Icon, color = 'var(--ice)', children, action }: { title: string; icon: any; color?: string; children: React.ReactNode; action?: React.ReactNode }) {
  return (
...
// ── Dashboard ─────────────────────────────────────────────────────────────────
type Tab = 'overview' | 'maintenance' | 'site' | 'rules' | 'punishments' | 'warns' | 'definitions' | 'services' | 'staffguide'
...
function Dashboard({ user, onLogout }: { user: DiscordUser; onLogout: () => void }) {
  ...
  const saveAll = () => {
    ...
    saveReport(report); saveServices(services)
  ...
  const hardReset = () => {
    ...
    setReport(getReport()); setServices(getServices()); setStaffGuides(getStaffGuides())
  ...
  return (
    <div style={{ display: 'flex', minHeight: '100vh', position: 'relative', zIndex: 1, background: '#060c1a' }}>
      ...
      {/* Sidebar */}
      <aside style={{ width: 210, flexShrink: 0, borderRight: '1px solid rgba(126,200,227,.07)', display: 'flex', flexDirection: 'column', position: 'sticky', top: 0, height: '100vh', overflowY: 'auto' }}>
        {/* Brand */}
        <div style={{ padding: '1.25rem 1rem 1rem', borderBottom: '1px solid rgba(126,200,227,.07)' }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '.6rem', marginBottom: '.9rem' }}>
            <div style={{ width: 30, height: 30, borderRadius: '.45rem', background: 'rgba(126,200,227,.12)', border: '1px solid rgba(126,200,227,.2)', display: 'grid', placeItems: 'center', flexShrink: 0 }}>
              <Settings size={14} color="var(--ice)" />
            </div>
            <div>
          ...
          {NAV.map(n => {
            ...
            return (
              <React.Fragment key={n.id}>
                {n.divider && <div style={{ height: '1px', background: 'rgba(126,200,227,.06)', margin: '.5rem .4rem' }} />}
                <button onClick={() => setTab(n.id)} style={{
                  ...
                  background: active ? 'rgba(126,200,227,.1)' : 'transparent',
                  color: active ? 'var(--ice2)' : 'var(--muted)',
        ...
        {/* Sidebar footer */}
        <div style={{ padding: '.75rem .6rem', borderTop: '1px solid rgba(126,200,227,.07)' }}>
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', minWidth: 0 }}>
        ...
        {/* Top bar */}
        <header style={{ height: 56, borderBottom: '1px solid rgba(126,200,227,.07)', display: 'flex', alignItems: 'center', justifyContent: 'space-between', padding: '0 1.75rem', background: 'rgba(6,12,26,.8)', backdropFilter: 'blur(20px)', position: 'sticky', top: 0, zIndex: 50 }}>
          <div style={{ display: 'flex', alignItems: 'center', gap: '.65rem' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: '.65rem' }}>
            ...
            <button onClick={saveAll} style={{ display: 'flex', alignItems: 'center', gap: '.4rem', fontFamily: 'Cinzel, serif', fontSize: '.68rem', letterSpacing: '.07em', padding: '.38rem 1rem', borderRadius: '.5rem', cursor: 'pointer', background: 'rgba(126,200,227,.15)', border: '1px solid rgba(126,200,227,.3)', color: 'var(--ice2)', transition: 'all .15s' }}>
        ...
        <main style={{ flex: 1, padding: '2rem 1.75rem 6rem', overflowY: 'auto' }}>
          ...
          {tab === 'services'    && <ServicesTab    services={services} setServices={wrap(setServices)} />}
...
function OverviewTab({ site, setSite, onWrap, flash, quickToggle, goTo }: any) {
  const stats = [
    { label: 'Rules', value: getRules().reduce((a, s) => a + s.fields.length, 0), icon: BookOpen, color: 'var(--ice)' },
    { label: 'Warn steps', value: getWarnSteps().length, icon: AlertTriangle, color: 'var(--red)' },
    { label: 'Services', value: getServices().length, icon: Zap, color: '#34d399' },
    ...
    <>
      ...
      <div style={{ background: site.maintenanceMode ? 'rgba(248,113,113,.06)' : 'rgba(74,222,128,.04)', border: `1px solid ${site.maintenanceMode ? 'rgba(248,113,113,.25)' : 'rgba(74,222,128,.2)'}`, borderRadius: '1rem', padding: '1.25rem 1.5rem', marginBottom: '1.5rem', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: '1rem', flexWrap: 'wrap' }}>
        ...
        <div style={{ display: 'flex', gap: '.65rem', flexWrap: 'wrap' }}>
          ...
          <button onClick={() => goTo('maintenance')} style={{ display: 'flex', alignItems: 'center', gap: '.4rem', fontFamily: 'Cinzel, serif', fontSize: '.68rem', letterSpacing: '.07em', padding: '.45rem 1rem', borderRadius: '.5rem', cursor: 'pointer', border: '1px solid rgba(126,200,227,.2)', background: 'rgba(126,200,227,.07)', color: 'var(--ice)' }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(160px, 1fr))', gap: '.75rem', marginBottom: '1.75rem' }}>
        {stats.map(s => (
          <button key={s.label} onClick={() => goTo(s.label === 'Rules' ? 'rules' : s.label === 'Warn steps' ? 'warns' : s.label === 'Services' ? 'services' : 'staffguide')} style={{ background: 'rgba(255,255,255,.02)', border: '1px solid rgba(255,255,255,.05)', borderRadius: '.85rem', padding: '1rem 1.1rem', textAlign: 'left', cursor: 'pointer', transition: 'all .15s' }}
      ...
      <Section title="Quick Actions" icon={Zap}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(200px, 1fr))', gap: '.6rem' }}>
          {[
            { label: 'Edit Server Info',     tab: 'site',        icon: Settings,      color: 'var(--ice)' },
            { label: 'Manage Rules',         tab: 'rules',       icon: BookOpen,      color: 'var(--ice)' },
    ...
    <>
      ...
      <div style={{ background: site.maintenanceMode ? 'rgba(248,113,113,.05)' : 'rgba(74,222,128,.03)', border: `1px solid ${site.maintenanceMode ? 'rgba(248,113,113,.25)' : 'rgba(74,222,128,.18)'}`, borderRadius: '1.1rem', padding: '1.5rem', marginBottom: '1.5rem' }}>
        ...
        {site.maintenanceMode && site.maintenanceAutoDisable && countdown && (
          <div style={{ display: 'flex', alignItems: 'center', gap: '.5rem', background: 'rgba(126,200,227,.06)', border: '1px solid rgba(126,200,227,.15)', borderRadius: '.55rem', padding: '.55rem .9rem' }}>
            <Timer size={13} color="var(--ice)" />
            <span style={{ fontFamily: 'Cinzel, serif', fontSize: '.7rem', color: 'var(--ice)', letterSpacing: '.05em' }}>Auto-disabling in {countdown}</span>
          </div>
        )}
      </div>

      {/* Schedule auto-disable */}
      <Section title="Schedule Auto-Disable" icon={Calendar} color="var(--ice)">
        <Toggle
          ...
          <>
            ...
            <div style={{ marginTop: '.65rem' }}>
              ...
              <div style={{ display: 'flex', gap: '.45rem', flexWrap: 'wrap' }}>
                {presets.map(p => (
                  <button key={p.label} onClick={() => applyPreset(p.ms)} style={{ fontFamily: 'Cinzel, serif', fontSize: '.63rem', letterSpacing: '.06em', padding: '.3rem .75rem', borderRadius: '.4rem', cursor: 'pointer', background: 'rgba(126,200,227,.06)', border: '1px solid rgba(126,200,227,.18)', color: 'var(--ice)', transition: 'all .15s' }}>
                    {p.label}
                  </button>
                ))}
              </div>
      ...
      <Section title="Maintenance Page Content" icon={FileText} color="var(--muted)">
        ...
        <div style={{ marginTop: '.75rem' }}>
          ...
          <div style={{ background: 'rgba(6,12,26,.8)', border: '1px solid rgba(126,200,227,.1)', borderRadius: '.85rem', padding: '1.5rem', textAlign: 'center' }}>
            ...
            <p style={{ fontFamily: '"Cinzel Decorative", serif', fontSize: '1.1rem', fontWeight: 700, background: 'linear-gradient(135deg,var(--ice3),var(--ice))', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', marginBottom: '.6rem' }}>{site.serverName}</p>
            <p style={{ fontSize: '.75rem', color: 'var(--dim)', maxWidth: 380, margin: '0 auto', lineHeight: 1.7 }}>{site.maintenanceMessage}</p>
            {site.maintenanceEta && (
              <div style={{ display: 'inline-flex', alignItems: 'center', gap: '.4rem', background: 'rgba(126,200,227,.06)', border: '1px solid rgba(126,200,227,.15)', borderRadius: '.5rem', padding: '.4rem .9rem', marginTop: '.85rem', fontSize: '.72rem', color: 'var(--ice)', fontFamily: 'Cinzel, serif' }}>
  ...
  return (
    <>
      ...
      <Section title="Discord / Social Embed" icon={Globe} color="rgba(88,101,242,.8)">
        ...
        <div style={{ background: '#1e1f22', borderRadius: '.65rem', padding: '1rem', border: '1px solid #3a3c41', borderLeft: '4px solid #7ec8e3', maxWidth: 440 }}>
      ...
      <Section title="Announcement Banner" icon={Bell} color="var(--gold)">
        ...
        {site.announcementEnabled && site.announcementBanner && (
          <div style={{ background: 'rgba(126,200,227,.08)', border: '1px solid rgba(126,200,227,.2)', borderRadius: '.45rem', padding: '.5rem .9rem', display: 'flex', alignItems: 'center', gap: '.5rem', marginTop: '.25rem' }}>
            <Bell size={12} color="var(--ice)" /><p style={{ fontSize: '.78rem', color: 'var(--ice3)', flex: 1 }}>{site.announcementBanner}</p><X size={13} color="var(--dim)" />
          </div>
        )}
      </Section>
    </>
          ...
          return (
            <div key={i} style={{ display:'grid',gridTemplateColumns:'38px 1fr auto auto',gap:'.65rem',alignItems:'center',background:`${color}08`,border:`1px solid ${color}20`,borderLeft:`3px solid ${color}`,borderRadius:'.55rem',padding:'.65rem .85rem',marginBottom:'.5rem' }}>
              ...
              <label style={{ display:'flex',alignItems:'center',gap:'.35rem',fontSize:'.68rem',color:'var(--dim)',cursor:'pointer',whiteSpace:'nowrap' }}>
                <input type="checkbox" checked={w.appealable} onChange={e=>u(i,{appealable:e.target.checked})} style={{ accentColor:'var(--ice)',width:14,height:14 }}/> Appealable
              </label>
...
// ── SERVICES TAB ──────────────────────────────────────────────────────────────
function ServicesTab({ services, setServices }: { services: ServiceItem[]; setServices: any }) {
  const u   = (i: number, p: Partial<ServiceItem>) => { const n=[...services]; n[i]={...n[i],...p}; setServices(n) }
  const del = (i: number) => setServices(services.filter((_,j)=>j!==i))
  const add = () => setServices([...services,{id:`s_${Date.now()}`,name:'New Service',tagline:'',description:'',badge:'',primaryLabel:'Website',primaryUrl:'https://',secondaryLabel:'',secondaryUrl:'',color:'#7ec8e3'}])
  return (
    <>
      <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'1.75rem' }}>
        <h2 style={{ fontFamily:'Cinzel, serif',fontSize:'1rem' }}>Services</h2>
        <Btn onClick={add} variant="primary" small><Plus size={12}/> Add Service</Btn>
      </div>
      {services.map((s,i) => (
        <div key={s.id} style={{ background:'rgba(255,255,255,.02)',border:`1px solid ${s.color}20`,borderLeft:`3px solid ${s.color}`,borderRadius:'.9rem',padding:'1.25rem',marginBottom:'.85rem' }}>
          <div style={{ display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:'1rem' }}>
            <p style={{ fontFamily:'Cinzel, serif',fontSize:'.82rem',color:s.color }}>{s.name}</p>
            <Btn onClick={()=>del(i)} variant="danger" small><Trash2 size={11}/> Remove</Btn>
          </div>
          <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:'.85rem' }}>
            <F label="Name"><input value={s.name} onChange={e=>u(i,{name:e.target.value})} style={css.input} onFocus={onFocus} onBlur={onBlur}/></F>
            <F label="Badge"><input value={s.badge} onChange={e=>u(i,{badge:e.target.value})} style={css.input} placeholder="e.g. Featured" onFocus={onFocus} onBlur={onBlur}/></F>
            <F label="Tagline"><input value={s.tagline} onChange={e=>u(i,{tagline:e.target.value})} style={css.input} onFocus={onFocus} onBlur={onBlur}/></F>
            <F label="Colour">
              <div style={{ display:'flex',gap:'.5rem' }}>
                <input type="color" value={s.color} onChange={e=>u(i,{color:e.target.value})} style={{ width:38,height:34,padding:'.1rem',background:'#0d1428',border:'1px solid rgba(255,255,255,.1)',borderRadius:'.4rem',cursor:'pointer',flexShrink:0 }}/>
                <input value={s.color} onChange={e=>u(i,{color:e.target.value})} style={{...css.input,flex:1}} onFocus={onFocus} onBlur={onBlur}/>
              </div>
            </F>
          </div>
          <F label="Description"><textarea value={s.description} onChange={e=>u(i,{description:e.target.value})} style={css.ta} onFocus={onFocus} onBlur={onBlur}/></F>
          <div style={{ display:'grid',gridTemplateColumns:'1fr 1fr',gap:'.85rem' }}>
            <F label="Primary Button Label"><input value={s.primaryLabel} onChange={e=>u(i,{primaryLabel:e.target.value})} style={css.input} onFocus={onFocus} onBlur={onBlur}/></F>
            <F label="Primary Button URL"><input value={s.primaryUrl} onChange={e=>u(i,{primaryUrl:e.target.value})} style={css.input} onFocus={onFocus} onBlur={onBlur}/></F>
            <F label="Secondary Button Label"><input value={s.secondaryLabel} onChange={e=>u(i,{secondaryLabel:e.target.value})} style={css.input} placeholder="Leave blank to hide" onFocus={onFocus} onBlur={onBlur}/></F>
            <F label="Secondary Button URL"><input value={s.secondaryUrl} onChange={e=>u(i,{secondaryUrl:e.target.value})} style={css.input} onFocus={onFocus} onBlur={onBlur}/></F>
          </div>
        </div>
      ))}
    </>
...
function StaffGuideTab({ staffGuides, setStaffGuides }: { staffGuides: StaffRoleGuide[]; setStaffGuides: any }) {
  ...
  const addRole = () => setStaffGuides([...staffGuides,{id:`r_${Date.now()}`,roleName:'New Role',color:'#7ec8e3',tier:'mod',summary:'Role summary.',sections:[]}])
  ...
  if (loading) return (
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', position: 'relative', zIndex: 1 }}>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem' }}>
        <Snowflake size={32} color="var(--ice)" style={{ animation: 'loaderSpin 2s linear infinite' }} />
