...
const tierColors: Record<string, string> = {
  policy: '#7ec8e3', leadership: '#f59e0b', admin: '#f472b6', mod: '#34d399',
}
...
const noteCategoryMeta = {
  general:      { label: 'General',      color: '#7ec8e3', icon: MessageSquare },
...
const incidentCategoryMeta = {
  member: { label: 'Member',        color: '#7ec8e3' },
  ...
  return (
    <div style={{ minHeight: '100vh', display: 'grid', placeItems: 'center', padding: '2rem' }}>
      <div style={{ maxWidth: 420, width: '100%', textAlign: 'center' }}>
        ...
        <h1 style={{ fontFamily: '"Cinzel Decorative", serif', fontSize: '1.5rem', fontWeight: 700, marginBottom: '.5rem', background: 'linear-gradient(135deg,var(--ice3),var(--ice))', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent' }}>Staff Portal</h1>
...
function NoteComposer({ user, editNote, onSave, onCancel }: { user: StaffUser; editNote: StaffNote | null; onSave: (content: string, category: NoteCategory) => void; onCancel: () => void }) {
  ...
  return (
    <div style={{ background: 'var(--bg2)', border: '1px solid rgba(126,200,227,.2)', borderRadius: '.8rem', padding: '1rem', marginBottom: '1rem' }}>
    ...
    <div>
      <div style={{ display: 'flex', gap: '.5rem', flexWrap: 'wrap', alignItems: 'center', marginBottom: '1rem' }}>
        <div style={{ display: 'flex', gap: '.35rem', flexWrap: 'wrap', flex: 1 }}>
          <button onClick={() => setFilterCat('all')} style={{ fontSize: '.65rem', fontFamily: 'Cinzel, serif', padding: '4px 10px', borderRadius: '.35rem', cursor: 'pointer', background: filterCat === 'all' ? 'rgba(126,200,227,.15)' : 'transparent', border: `1px solid ${filterCat === 'all' ? 'rgba(126,200,227,.35)' : 'var(--border)'}`, color: filterCat === 'all' ? 'var(--ice)' : 'var(--muted)' }}>All</button>
          {(Object.entries(noteCategoryMeta) as [NoteCategory, typeof noteCategoryMeta[NoteCategory]][]).map(([key, m]) => {
            const CatIcon = m.icon
        ...
        <button onClick={() => { setComposing(true); setEditNote(null) }} style={{ display: 'inline-flex', alignItems: 'center', gap: '.3rem', fontSize: '.68rem', fontFamily: 'Cinzel, serif', padding: '.4rem .9rem', borderRadius: '.45rem', cursor: 'pointer', background: 'rgba(126,200,227,.12)', border: '1px solid rgba(126,200,227,.3)', color: 'var(--ice)' }}><Plus size={13} /> New Note</button>
      </div>
...
function LOAForm({ user, userRoles, editEntry, onSave, onCancel }: { user: StaffUser; userRoles: string[]; editEntry: LOAEntry | null; onSave: (f: Omit<LOAEntry, 'id' | 'createdAt' | 'returnedAt'>) => void; onCancel: () => void }) {
  ...
  return (
    <div style={{ background: 'var(--bg2)', border: '1px solid rgba(126,200,227,.2)', borderRadius: '.8rem', padding: '1.1rem', marginBottom: '1rem' }}>
      ...
      <div style={{ display: 'flex', gap: '.4rem', marginBottom: '.9rem', flexWrap: 'wrap' }}>
        {(['loa','partial'] as const).map(t => <button key={t} onClick={() => setType(t)} style={{ fontSize: '.65rem', fontFamily: 'Cinzel, serif', padding: '5px 12px', borderRadius: '.35rem', cursor: 'pointer', background: type === t ? 'rgba(126,200,227,.15)' : 'transparent', border: `1px solid ${type === t ? 'rgba(126,200,227,.35)' : 'var(--border)'}`, color: type === t ? 'var(--ice)' : 'var(--muted)' }}>{t === 'loa' ? 'Full LOA (5+ days)' : 'Short Absence (1–4 days)'}</button>)}
      </div>
      ...
      <div style={{ background: 'rgba(126,200,227,.05)', border: '1px solid rgba(126,200,227,.15)', borderRadius: '.5rem', padding: '.65rem .8rem', marginBottom: '.9rem', fontSize: '.72rem', color: 'var(--dim)', lineHeight: 1.65 }}>
        <strong style={{ color: 'var(--ice)' }}>Reminder:</strong> {type === 'loa' ? 'Full LOAs require approval from your direct superior. Also post in #staff-loa on Discord.' : 'Short absences: just let your team know in staff chat.'}
      </div>
      <div style={{ display: 'flex', gap: '.5rem', justifyContent: 'flex-end' }}>
        <button onClick={onCancel} style={{ display: 'inline-flex', alignItems: 'center', gap: '.3rem', fontSize: '.7rem', fontFamily: 'Cinzel, serif', padding: '.4rem .9rem', borderRadius: '.45rem', cursor: 'pointer', background: 'transparent', border: '1px solid var(--border)', color: 'var(--muted)' }}><X size={12} /> Cancel</button>
        <button onClick={() => onSave({ authorId: user.id, authorName: user.username, authorAvatar: user.avatar, authorRole: hr?.name ?? 'Staff', authorRoleColor: hr?.color ?? '#7ec8e3', type, from, to, reason, coverage, status: 'active' })} style={{ display: 'inline-flex', alignItems: 'center', gap: '.3rem', fontSize: '.7rem', fontFamily: 'Cinzel, serif', padding: '.4rem .9rem', borderRadius: '.45rem', cursor: 'pointer', background: 'rgba(126,200,227,.15)', border: '1px solid rgba(126,200,227,.3)', color: 'var(--ice)' }}><Check size={12} /> {editEntry ? 'Update' : 'Submit'}</button>
      </div>
    </div>
  )
}
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(140px, 1fr))', gap: '.65rem', marginBottom: '1.5rem' }}>
        {[
          ...
          { label: 'This Month',    value: entries.filter(e => new Date(e.createdAt).getMonth() === new Date().getMonth()).length, color: '#7ec8e3', icon: Clock },
        ].map(s => {
          const SIcon = s.icon
          return (
      ...
      <div style={{ display: 'flex', gap: '.5rem', alignItems: 'center', marginBottom: '1rem', flexWrap: 'wrap' }}>
        <div style={{ display: 'flex', background: 'var(--bg2)', border: '1px solid var(--border)', borderRadius: '.5rem', padding: '.2rem', gap: '.2rem' }}>
          <button onClick={() => setHistoryView(false)} style={{ fontSize: '.65rem', fontFamily: 'Cinzel, serif', padding: '4px 12px', borderRadius: '.35rem', cursor: 'pointer', background: !historyView ? 'var(--bg4)' : 'transparent', border: `1px solid ${!historyView ? 'rgba(126,200,227,.2)' : 'transparent'}`, color: !historyView ? 'var(--ice)' : 'var(--muted)' }}>Active / Upcoming</button>
          <button onClick={() => setHistoryView(true)}  style={{ fontSize: '.65rem', fontFamily: 'Cinzel, serif', padding: '4px 12px', borderRadius: '.35rem', cursor: 'pointer', background:  historyView ? 'var(--bg4)' : 'transparent', border: `1px solid ${ historyView ? 'rgba(126,200,227,.2)' : 'transparent'}`, color:  historyView ? 'var(--ice)' : 'var(--muted)' }}>History</button>
        </div>
        <button onClick={() => { setShowForm(true); setEditEntry(null) }} style={{ marginLeft: 'auto', display: 'inline-flex', alignItems: 'center', gap: '.3rem', fontSize: '.68rem', fontFamily: 'Cinzel, serif', padding: '.4rem .9rem', borderRadius: '.45rem', cursor: 'pointer', background: 'rgba(126,200,227,.12)', border: '1px solid rgba(126,200,227,.3)', color: 'var(--ice)' }}><Plus size={13} /> Submit LOA</button>
      </div>
...
function IncidentsPanel({ user, isAdmin }: { user: StaffUser; isAdmin: boolean }) {
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: '.5rem', flexWrap: 'wrap', alignItems: 'center', marginBottom: '1rem' }}>
        <div style={{ display: 'flex', gap: '.35rem', flexWrap: 'wrap', flex: 1 }}>
          <button onClick={() => setFilterStatus('all')} style={{ fontSize: '.65rem', fontFamily: 'Cinzel, serif', padding: '4px 10px', borderRadius: '.35rem', cursor: 'pointer', background: filterStatus === 'all' ? 'rgba(126,200,227,.15)' : 'transparent', border: `1px solid ${filterStatus === 'all' ? 'rgba(126,200,227,.35)' : 'var(--border)'}`, color: filterStatus === 'all' ? 'var(--ice)' : 'var(--muted)' }}>All</button>
      ...
      {viewInc && (
        <Modal title={viewInc.title} onClose={() => setViewInc(null)}>
          ...
          <div style={{ display: 'flex', gap: '.5rem' }}>
            ...
            <button onClick={() => addNote(viewInc.id)} style={{ display: 'inline-flex', alignItems: 'center', gap: '.3rem', fontSize: '.7rem', fontFamily: 'Cinzel, serif', padding: '.45rem .9rem', borderRadius: '.45rem', cursor: 'pointer', background: 'rgba(126,200,227,.15)', border: '1px solid rgba(126,200,227,.3)', color: 'var(--ice)', whiteSpace: 'nowrap' }}><Check size={12} /> Add</button>
          </div>
        </Modal>
      )}
    </div>
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: '.35rem' }}>
            {entries.map(entry => (
              <div key={entry.id} style={{ display: 'flex', alignItems: 'flex-start', gap: '.65rem', background: 'var(--bg1)', border: '1px solid var(--border)', borderRadius: '.6rem', padding: '.7rem .9rem' }}>
                <Activity size={13} color="var(--ice)" style={{ flexShrink: 0, marginTop: 2 }} />
                <div style={{ flex: 1, minWidth: 0 }}>
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: '.75rem', alignItems: 'center', marginBottom: '1rem', flexWrap: 'wrap' }}>
        ...
        <button onClick={() => setShowEdit(true)} style={{ display: 'inline-flex', alignItems: 'center', gap: '.3rem', fontSize: '.68rem', fontFamily: 'Cinzel, serif', padding: '.4rem .9rem', borderRadius: '.45rem', cursor: 'pointer', background: 'rgba(126,200,227,.12)', border: '1px solid rgba(126,200,227,.3)', color: 'var(--ice)' }}><Pencil size={12} /> Edit My Profile</button>
      </div>
...
function LinksPanel({ isAdmin }: { isAdmin: boolean }) {
  const links = [
    ...
    { label: 'Public Rules',     url: '/rules',                                     color: '#7ec8e3', desc: 'Current live server rules' },
    ...
    <div style={{ position: 'relative', zIndex: 1, maxWidth: 1100, margin: '0 auto', padding: '2rem 1.25rem 5rem' }}>
      ...
      <div style={{ display: 'flex', alignItems: 'flex-start', justifyContent: 'space-between', gap: '1rem', flexWrap: 'wrap', marginBottom: '1.75rem' }}>
        <div>
          ...
          <h1 style={{ fontFamily: '"Cinzel Decorative", serif', fontSize: 'clamp(1.5rem, 4vw, 2.2rem)', fontWeight: 700, letterSpacing: '-.01em', background: 'linear-gradient(135deg,var(--ice3),var(--ice),var(--gold))', WebkitBackgroundClip: 'text', WebkitTextFillColor: 'transparent', marginBottom: '.4rem' }}>Staff Portal</h1>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '.75rem', background: 'var(--bg1)', border: '1px solid var(--border)', borderRadius: '.75rem', padding: '.65rem .9rem' }}>
          ...
          <div style={{ minWidth: 0 }}>
            <p style={{ fontFamily: 'Cinzel, serif', fontSize: '.74rem', fontWeight: 700, color: highestRole?.color ?? 'var(--ice)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis', maxWidth: 140 }}>{user.username}</p>
  ...
  if (state === 'loading') return (
    <div style={{ minHeight: '100vh', display: 'grid', placeItems: 'center' }}>
      <div style={{ display: 'flex', flexDirection: 'column', alignItems: 'center', gap: '1rem' }}>
        <div style={{ width: 40, height: 40, border: '2px solid rgba(126,200,227,.2)', borderTop: '2px solid var(--ice)', borderRadius: '50%', animation: 'spin 1s linear infinite' }} />
